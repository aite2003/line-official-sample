...
@IonicPage()
@Component({
  selector: 'page-pick-origin',
  templateUrl: 'pick-origin.html',
})
export class PickOriginPage {
  maps:any;
...
saveLatLong()
{
  if(this.latlong)
  {
    localStorage.setItem(this.mode+'-latlong',this.latlong);
...
saveDetail()
{
  ...
  {
    localStorage.setItem(this.mode+'-detail',this.locationDetail+" "+this.locationName);
    //this.state="location-detail";
    this.navCtrl.pop();
  }
  else
